...
y= df['class']
############################################
x_train, x_test, y_train, y_test = tts(x,y,test_size=0.3,random_state=0) # create random train and test variables from x and y
############################################
ns = knn(n_neighbors=3) # call Knn.- we have 3 class so n_neighbors is 3
...
f = open('knn_accuracy score.txt','w') # to open or create a txt file
f.write(acc_sc) # write a variable in the opened file
f.close() # to stop writing on the opened file

np.savetxt('knn_confusion matrix.txt',conf_mat, fmt = '%10.5f') # save confusion matrix 

f = open('knn_IoU score.txt','w')
f.write(jac_sc)
f.close()

f = open('knn_precision score.txt','w')
f.write(prec_sc)
f.close()

pred_results.to_csv('knn_results.csv') # create a csv file for real y test data and prediction of it